...
%     [ p e ef efe ] = qsimvnef( 50000, r, a, b, f ); disp([ p ef; e efe ])
%   Note: if f is defined in an m-file f.m, then use
%     [p e ef efe] = qsimvnef( 50000, r, a, b, 'f' ); disp([ p ef; e efe ]) 
%    which is usually faster than the inline f.
%   Note: if f is vector valued, it must be a row vector; example
%     f = inline('[x(1) x(2) x(3) x(4)]','x');
%     [ p e ef efe ] = qsimvnef( 50000, r, a, b, f ); disp([ p ef; e efe ])
%
...
% Initialization
%
% Michael Osborne has edited the line below, ch used to take the cholesky
% decomp of r, now it doesn;t
...
ns = 10; nv = max( fix(m/ns), 1 ); 
ps = sqrt(primes(5*(n+1)*log(n+1)/4)); 
if isempty(ps);ps=1;end;
q = ps(1:n)'; % Richtmyer Sequence
%
% Randomization loop for ns samples
%
for i = 1 : ns, vi = 0; xr = rand( n, 1 ); 
  %
