...
if (~outputGP && nargout == 1) || (outputGP && nargout <= 3)
    [m,D]=posterior_spgp(XStar,gp,sample,...
        'var_not_cov');
elseif (~outputGP && nargout == 2) || (outputGP && nargout <= 6)
    [m,D,gm,gD]=posterior_spgp(XStar,gp,sample,...
        'var_not_cov');
elseif (~outputGP && nargout == 3) || (outputGP && nargout <= 9)
    [m,D,gm,gD,Hm,HD]=posterior_spgp(XStar,gp,sample,...
        'var_not_cov'); 
end
...
if (~outputGP && nargout > 1) || (outputGP && nargout > 3)  
    ...
    g = 0.5*bsxfun(@times,gm,1+erf(argterm))...
        -0.5*bsxfun(@times,gD,(2*pi*D).^(-0.5).*exp(-argterm.^2));
