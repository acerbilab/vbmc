function logP = logP_ring_topo(theta, direction, R, K,  p_pulse, decay, q)
...
if iscell(theta)
    logP = 0;
    for i = 1:numel(theta)
        logP = logP + logP_ring_topo(theta{i}, direction{i}, R, K, p_pulse, decay, q);
    end
else
...
for count = 1:length(direction)-1
    ...
    for i = 1:N
        ...
        [nnsR, nnsidx] = sort(gap);
        
        sd =0;
        
        for j = 1:round(K)
               
                if direction(i, count) == -direction(nnsidx(j+1), count)
                    sd = sd + p_pulse(1);
                end
        end
