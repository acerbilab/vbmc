function [t, theta, direction] = simulate_ring_R(N, R, K, p_pulse, decay, q)

K = min(K, N-1); %Can't interact with more neighbours than exist
    ...
    for ri = 1:N
       ...
       [nnsR, nnsidx] = sort(gap);
        ...
        for j = 1:round(K)
                ...
                if direction(i) == -direction(nnsidx(j+1))
                    sd = sd + p_pulse(1);
                end
        end
