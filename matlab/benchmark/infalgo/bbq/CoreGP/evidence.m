function [mean_evidence, scaled_var_evidence] = ...
    evidence(hs_t, sample_struct, prior_struct, r_gp, opt)
% returns the expected mean evidence and its (scaled) variance after adding
% a hyperparameter sample hs_t. 
%
% [mean_evidence, scaled_var_evidence] = ...
% evidence(hs_t, sample_struct, prior_struct, r_gp, opt)
% - hs_t (1 by m) is a row vector expressing the relevant trial
% hyperparameters. If hs_t is empty or omitted, then the returned mean
% and variance are the non-expected qantities for the evidence.
...
not_expected = isstruct(hs_t) || isempty(hs_t);

if isstruct(hs_t)
    % hs_t has been omitted, move all arguments along one.
    % (hs_t, sample_struct, prior_struct, r_gp, opt)
