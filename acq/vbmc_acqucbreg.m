function acq = vbmc_acqucbreg(Xs,vp,gp,optimState,Nacq,transpose_flag)
...
% nu = max(1,log(size(gp.X,1)));
betat = 2/5*log(size(gp.X,1)^2*pi^2/0.6);
acq = -vtot .* exp((fbar+betat*sqrt(vtot)-z)) .* p;
