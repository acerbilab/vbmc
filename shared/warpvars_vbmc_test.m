function warpvars_vbmc_test(nvars)

if nargin < 1 || isempty(nvars); nvars = 1; end


for iType = [0,3,9,10,12,13]
    ...
    trinfo = warpvars_vbmc(nvars,LB,UB);
    ...
    x2 = warpvars_vbmc(warpvars_vbmc(x,'dir',trinfo),'inv',trinfo);    
...
%     x0t = warpvars_vbmc(x0,'dir',trinfo);
%     
%     derivcheck(@(x) fun(x,trinfo),x0,1);
...
y = warpvars_vbmc(x,'dir',trinfo);
% dy = warpvars_vbmc(y,'g',trinfo);

if logflag
    dy = exp(-warpvars_vbmc(y,'logpdf',trinfo));
else
    dy = 1./warpvars_vbmc(y,'pdf',trinfo);
end
...
y = warpvars_vbmc(x,'inv',trinfo);
dy = warpvars_vbmc(x,'r',trinfo);
% dy = exp(-warpvars_vbmc(y,'logpdf',trinfo));
...
y = warpvars_vbmc(x,'d',trinfo);
dy = warpvars_vbmc(y,'f',trinfo); 
