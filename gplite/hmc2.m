...
% Scaling for each parameter (aka diagonal mass matrix)
if isempty(opt_widths)
    opt_widths = ones(1,nparams);
end

step_sizes = opt_widths;
inv_mass_matrix = diag(opt_widths.^2);
...
while k <= opt_nsamples    %samples from k>0
  ...
  Eold = E;
  Hold = E + 0.5*(p*inv_mass_matrix*p');
