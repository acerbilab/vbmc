function [samples,fvals,exitflag,output] = slicelite(logf,x0,N,widths,LB,UB,options,varargin)
%SLICELITE Lite coordinate-wise slice sampling method.
%
...
if isempty(options.LogPrior); doprior = false; else; doprior = true; end
...
% First evaluations at X0
y = zeros(K,1); logprior = zeros(K,1);
[y(1),fval_temp,logprior(1)] = feval(@logpdfbound, x0(1,:), varargin{:});
fval = zeros(K,numel(fval_temp)); fval(1,:) = fval_temp(:)';
for k = 2:K
    [y(k),fval(k,:),logprior(k)] = feval(@logpdfbound, x0(k,:), varargin{:});
end
xx = x0;
samples = zeros(N,D);
if ~isempty(options.LogPrior) && nargout > 3; logpriors = zeros(N, 1); else; logpriors = []; end
if nargout > 1; fvals = zeros(N, size(fval,2)); end
...
for ii = 1:(effN+burn)
    ...
    for idd = 1:D
        ...
        rr = rand;
        x_l(dd) = xx(kk,dd) - rr*widths(dd);
        x_r(dd) = xx(kk,dd) + (1-rr)*widths(dd);
        ...
        while 1
            ...
            [log_Px(kk),fval(kk,:),logprior(kk)] = feval(@logpdfbound, xprime, varargin{:});
                        
            if log_Px(kk) > log_uprime
                break % this is the only way to leave the while loop
            else
                % Shrink in
                if xprime(dd) > xx(kk,dd)
                    x_r(dd) = xprime(dd);
                elseif xprime(dd) < xx(kk,dd)
                    x_l(dd) = xprime(dd);
                else
    ...
    if record
        ...
        samples(ismpl,:) = xx(kk,:);
        if nargout > 1; fvals(ismpl,:) = fval(kk,:); end
        if nargout > 3 && doprior; logpriors(ismpl) = logprior(kk); end
    end
